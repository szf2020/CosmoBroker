...
public class ClusterManager
{
    ...
    private readonly List<IPEndPoint> _gateways = new();
    private readonly ConcurrentDictionary<IPEndPoint, BrokerConnection> _gatewayConnections = new();
    ...
    public void AddGateway(IPEndPoint endPoint)
    {
        _gateways.Add(endPoint);
    }

    public async Task StartAsync(CancellationToken ct)
    {
        foreach (var peer in _peers) _ = ConnectToPeerAsync(peer, ct);
        foreach (var gw in _gateways) _ = ConnectToGatewayAsync(gw, ct);
    }
    ...
    private async Task ConnectToGatewayAsync(IPEndPoint peer, CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                var connection = new BrokerConnection(stream, peer.ToString(), _topicTree, null, null, null, _server, sendInfoOnConnect: false);
                connection.IsRoute = true;
                _gatewayConnections[peer] = connection;

                // Identify as gateway (route=true for now)
                var connect = new Auth.ConnectOptions { Route = true };
                string json = System.Text.Json.JsonSerializer.Serialize(connect);
                await connection.SendRawAsync($"CONNECT {json}\r\n");
            ...
            {
                Console.WriteLine($"[ClusterManager] Gateway error connecting to {peer}: {ex.Message}. Retrying in 5s...");
                try { await Task.Delay(5000, ct); } catch { break; }
            }
            finally
            {
                _gatewayConnections.TryRemove(peer, out _);
            }
        }
    }
    ...
    {
        foreach (var conn in _connections.Values.Concat(_gatewayConnections.Values))
        {
